...
static void insercao(Jogador* array[], int n, int k, int *comp, int *mov){
        int i, j;
        for(i = 1; i < n; i++){
            Jogador *temp;
            temp = (Jogador*) malloc(sizeof(Jogador));
            clone(array[i], temp);
            (*mov)++;
            j = (i < k)? i - 1: k - 1;
            (*comp) += 3;
            while((j >=0) && ((array[j]->anoNascimento > temp -> anoNascimento) || (array[j] -> anoNascimento == temp -> anoNascimento &&(strcmp(array[j] -> nome, temp -> nome) > 0)))){
            clone(array[j], array[j + 1]);
            j--;
            (*comp) += 3;
            (*mov)++;
            }
            clone(temp, array[j + 1]);
            (*mov)++;
        }
    }
...
int main(){
    ...
    insercao(entrada, realTam, 10, &comp, &mov);
    clock_t fim = clock();
